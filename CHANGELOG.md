...
- **Bulk-ingest modules**: a new module shape for datasets published as bulk
  ...
  central-directory reader handles single- and multi-entry ZIPs) and indexes it
  into a local SQLite database (built-in `node:sqlite`, no native dep; requires
  Node ≥ 22.5, degrades with a clear message), then serves fast local queries.
  - `form5500` — a company's 401(k)/pension/welfare plans by sponsor EIN or name
    (~226K filings/year from a ~29MB DOL download).
