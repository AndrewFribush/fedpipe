...
function resolveSubject(subjectInput: string): { subject: string; subscale: string } {
  ...
  const entry = (SUBJECTS as Record<string, (typeof SUBJECTS)[keyof typeof SUBJECTS]>)[lower];
  ...
  if (aliasKey && (SUBJECTS as Record<string, unknown>)[aliasKey]) {
    const resolved = (SUBJECTS as Record<string, (typeof SUBJECTS)[keyof typeof SUBJECTS]>)[aliasKey!];
