...
/**
 * The CDE summarized-crime / arrest chart payload nests the offense series
 * ({offenses:{rates,actuals}}) alongside pure chart-rendering cruft — `tooltips`
 * (axis labels, coverage %) and `populations` (per-month denominators). Keep the
 * offenses and the tiny cde_properties (data dates); drop the rest so the
 * response is the actual crime numbers, not a chart spec.
 */
function digestCrimeChart(node: unknown): unknown {
  if (!node || typeof node !== "object" || Array.isArray(node)) return digestSeries(node);
  const src = node as Record<string, unknown>;
  // Only prune the offense-chart shape; anything else keeps the full digest so
  // we never accidentally drop a tool's actual data.
  if (!("offenses" in src)) return digestSeries(node);
  const kept: Record<string, unknown> = { offenses: digestSeries(src.offenses) };
  if ("cde_properties" in src) kept.cde_properties = digestSeries(src.cde_properties);
  return kept;
}
...
export const tools: Tool<any, any>[] = [
  ...
  // ── 2. Summarized Crime ──────────────────────────────────────────
  {
    ...
    execute: async ({ offense, state, ori, from_year, to_year }) => {
      ...
      return recordResponse(`FBI summarized crime (${offense}) at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
  ...
  // ── 3. Arrest Data ───────────────────────────────────────────────
  {
    ...
    execute: async ({ offense, state, ori, type, from_year, to_year }) => {
      ...
      return recordResponse(`FBI arrest data (${offense}) at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
  ...
  // ── 4. Expanded Homicide ─────────────────────────────────────────
  {
    ...
    execute: async ({ state, ori, type, from_year, to_year }) => {
      ...
      return recordResponse(`FBI expanded homicide data at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
  ...
  // ── 5. Hate Crime ────────────────────────────────────────────────
  {
    ...
    execute: async ({ state, ori, bias, type, from_year, to_year }) => {
      ...
      return recordResponse(`FBI hate crime data at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
  ...
  // ── 9. NIBRS (Incident-Based) ────────────────────────────────────
  {
    ...
    execute: async ({ offense, state, ori, type, from_year, to_year }) => {
      ...
      return recordResponse(`FBI NIBRS data (${offense}) at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
  ...
  // ── 10. Expanded Property (Supplemental) ─────────────────────────
  {
    ...
    execute: async ({ offense, state, ori, type, from_year, to_year }) => {
      ...
      return recordResponse(`FBI expanded property (${offense}) at ${level} level`, digestCrimeChart(data) as Record<string, unknown>);
    },
  },
];
